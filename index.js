let rf = document.getElementById("user_form");
const save_data = (x) => {
    x.preventDefault();
...
const display_data = () => {
    ...
    const tableEntries = entries.map((entry) => {
        const nameCell = `<td class='border px-4 py-2'>${entry.m}</td>`;
        const emailCell = `<td class='border px-4 py-2'>${entry.n}</td>`;
        const passwordCell = `<td class='border px-4 py-2'>${entry.o}</td>`;
...
rf.addEventListener("submit", save_data);
